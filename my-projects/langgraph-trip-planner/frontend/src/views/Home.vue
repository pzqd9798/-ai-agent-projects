<!-- 1、template模板部分（页面结构） -->
<!-- 2、script脚本部分（逻辑与行为） -->
<!-- 3、style scoped样式部分（外观）-->

<template>
  <!-- 最外层页面容器，控制整个页面的背景和布局 -->
...
// Vue Composition API: ref 用于单个响应式值，reactive 用于对象响应式，watch 用于观察数据变化
import { ref, reactive, watch } from 'vue'
// Vue Router: 用于页面跳转
import { useRouter } from 'vue-router'
// Ant Design Vue: 用于提示消息显示
import { message } from 'ant-design-vue'
// 后端接口调用函数
import { generateTripPlan } from '@/services/api'
// 类型定义，用于 TypeScript 检查
import type { TripFormData } from '@/types'
import type { Dayjs } from 'dayjs'

// 获取路由实例，用于成功提交后跳转到结果页
...
const formData = reactive<Omit<TripFormData, 'start_date' | 'end_date'> & { start_date: Dayjs | null; end_date: Dayjs | null }>({
  city: '', // 目的地城市
  start_date: null, // 开始日期，Dayjs 对象或 null
  end_date: null, // 结束日期，Dayjs 对象或 null
  travel_days: 1, // 旅行天数，默认 1
  transportation: '公共交通', // 交通方式初始值
  accommodation: '经济型酒店', // 住宿偏好初始值
  preferences: [], // 旅行偏好标签数组
  free_text_input: '' // 用户自由输入的额外要求
})

// 监听开始日期和结束日期变化，自动计算旅行天数
watch([() => formData.start_date, () => formData.end_date], ([start, end]) => {
  if (start && end) {
    ...
    if (days > 0 && days <= 30) {
      ...
      // 超过 30 天的情况，提示并清空结束日期
      message.warning('旅行天数不能超过30天')
      formData.end_date = null
    } else {
      // 如果结束日期早于开始日期，提示并清空结束日期
...
const handleSubmit = async () => {
  ...
  const progressInterval = setInterval(() => {
    ...
    if (p <= 30) {
      ...
      loadingStatus.value = '📋 正在生成行程计划（LLM 思考中，请耐心等待）...'
    }
  }, 600)
  ...
  try {
    ...
    // 发送请求给后端，传入 signal 以支持取消
    const response = await generateTripPlan(requestData, abortController.value.signal)
