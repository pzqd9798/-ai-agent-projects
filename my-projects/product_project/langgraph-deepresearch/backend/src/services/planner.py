...
def _parse_tool_call_body(text: str, start_pos: int) -> tuple[str, str] | None:
    """Parse a single [TOOL_CALL:tool:body] using bracket counting.

    Returns (tool_name, body) or None.
    """
    m = TOOL_CALL_START.search(text, start_pos)
    if not m:
        return None
    ...
    for i in range(body_start, len(text)):
        ...
        if ch in ("{", "["):
            ...
            bracket_depth -= 1
            if bracket_depth == 0:
                return tool_name, text[body_start:i + 1]
...
class PlanningService:
    ...
    def _extract_tool_payload(self, text: str) -> Optional[dict[str, Any]]:
        ...
        parsed = _parse_tool_call_body(text, 0)
        if parsed is None:
            return None

        _tool_name, body = parsed
