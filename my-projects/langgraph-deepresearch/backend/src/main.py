...
Rewritten: streaming endpoint now uses async SSE via LangChain + asyncio.
"""
...
def create_app() -> FastAPI:
    app = FastAPI(title="HelloAgents Deep Researcher (LangChain Edition)")
    ...
    def log_startup_configuration() -> None:
        ...
        logger.info(
            "DeepResearch configuration loaded: provider=%s model=%s base_url=%s "
            "search_api=%s max_loops=%s fetch_full_page=%s strip_thinking=%s api_key=%s",
            config.llm_provider,
